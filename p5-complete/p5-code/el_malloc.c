...
el_blockhead_t *el_get_header(el_blockfoot_t *foot){
  ...
  el_blockhead_t *head = PTR_MINUS_BYTES(foot, sizeof(el_blockhead_t) + size); // use macro to get the previous head
  return head;
}
...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  el_blockfoot_t *foot = PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t)); // get the foot below the current block
  if((void *) foot < (void*) el_ctl->heap_start){ // if the block below is outside the heap return null
    return NULL;
  }
  el_blockhead_t *lower = el_get_header(foot);
  return lower;
}
...
void el_add_block_front(el_blocklist_t *list, el_blockhead_t *block){
  ...
  // adjust the byte size and the length of the list
...
void el_remove_block(el_blocklist_t *list, el_blockhead_t *block){
  // remove the block from the front of the list
...
el_blockhead_t *el_find_first_avail(size_t size){
  el_blockhead_t *block = el_ctl->avail->beg; // get the beginning of available blocks list
  while(block->state != EL_END_BLOCK){ // iterate through all the available blocks until the end is reached
    if(block->size >= size){ // if the current block has enough size, return it
      return block;
    }
    else{ // else make the current block the next block
      block = block->next;
    }
  }
  // if not block had enough space, return NULL
  return NULL;
}
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  if(block->size < (new_size + EL_BLOCK_OVERHEAD)){ // If the parameter block does not have sufficient size for a split
    return NULL; // return NULL
  }
  size_t old_size = block->size; // get the current size

  // adjust the old block
  block->size = new_size; // adjust the size of the head
  el_blockfoot_t *old_foot = el_get_footer(block); // get old block foot
  old_foot->size = new_size; // set the told foot size

  // create a new block
  el_blockhead_t *new_block = PTR_PLUS_BYTES(old_foot, sizeof(el_blockfoot_t)); // create a new block
  new_block->size = old_size - (EL_BLOCK_OVERHEAD + new_size);
  el_blockfoot_t *new_foot = el_get_footer(new_block); // get the foot of the 
  new_foot->size = new_block->size; // adjust the size of the new foot
...
void *el_malloc(size_t nbytes){
  el_blockhead_t *block = el_find_first_avail(nbytes); // find the first available block
  if(block == NULL){ // if no available block was found, return NULL
    return NULL; 
  }
  el_remove_block(el_ctl->avail, block); // remove the block from the available list
  el_blockhead_t *new_block = el_split_block(block, nbytes); // adjust the block, get a pointer to the available space
  if(new_block != NULL){ // if there was no split
    el_add_block_front(el_ctl->avail, new_block); // add the block to the used list
    new_block->state = EL_AVAILABLE;
  }
  if(block == NULL){ // if no available block was found, return NULL
    return NULL; 
  }
  block->state = EL_USED; // set the block state to used
  el_add_block_front(el_ctl->used, block); // add the block to the used list
...
void el_merge_block_with_above(el_blockhead_t *lower){
  if(lower == NULL || lower->state != EL_AVAILABLE){ // if lower is null or not EL_AVAILABLE, return
    return;
  }
  el_blockhead_t *higher = el_block_above(lower); // get block above
  if(higher == NULL || higher->state != EL_AVAILABLE){ // if higher is null or not EL_AVAILABLE, return
    return;
  }
  el_remove_block(el_ctl->avail, lower);
  el_remove_block(el_ctl->avail, higher);
  lower->size += (higher->size + EL_BLOCK_OVERHEAD); // add the size of the higher block
  el_blockfoot_t *foot = el_get_footer(higher); // get the foot
  foot->size = lower->size; // set the foot size
...
void el_free(void *ptr){
  el_blockhead_t *current = PTR_MINUS_BYTES(ptr, sizeof(el_blockhead_t)); // get head of the block
  if(current == NULL){ // if the head is NULL, return
    return;
  }
  el_remove_block(el_ctl->used, current); // remove block from the used list
  current->state = EL_AVAILABLE; // set the state to available
  el_add_block_front(el_ctl->avail, current); // add the block to the available list
  
  // check if the block can be merged with the block above
  el_blockhead_t *above = el_block_above(current);
  if(above != NULL && above->state == EL_AVAILABLE){ // if above is not null and is available
    el_merge_block_with_above(current); // merge with the block above
  }
  // check if the block can be merged with the block above
  el_blockhead_t *below = el_block_below(current); // get the block below
  if(below != NULL && below->state == EL_AVAILABLE){ // if below is not null and is available
    el_merge_block_with_above(below); // merge with the block below
  }
}
...
int el_append_pages_to_heap(int npages){
  void *heap =  // call mmap to expand the heap, starting the the address of the beginning of the heap, appending by npages * EL_PAGE_BYTES
    mmap(el_ctl->heap_end,
         (npages * EL_PAGE_BYTES),
         PROT_READ | PROT_WRITE,
         MAP_PRIVATE | MAP_ANONYMOUS,
         -1, 0);
  if(heap != el_ctl->heap_end){ // if mapping failed
    printf("ERROR: Unable to mmap() additional %d pages\n", npages);
    return 1;
  }
  el_ctl->heap_bytes += (npages * EL_PAGE_BYTES); // append the heap bytes
  el_ctl->heap_end = PTR_PLUS_BYTES( el_ctl->heap_end, (npages * EL_PAGE_BYTES)); // append the heap ending address
  el_blockhead_t *block = heap; // create a new block for the new allocated pages
  
  // set the fields of the new block and add it to the available list
  block->size = (npages * EL_PAGE_BYTES) - EL_BLOCK_OVERHEAD;
  el_blockfoot_t *foot = el_get_footer(block); // get the foot
  foot->size = (npages * EL_PAGE_BYTES) - EL_BLOCK_OVERHEAD; // set the size of the foot
  // set the state of the block to available and add to the available list
  block->state = EL_AVAILABLE;
  el_add_block_front(el_ctl->avail, block);

  // try to merge block
  el_blockhead_t *below = el_block_below(block); // get the block below
  if(below != NULL && below->state == EL_AVAILABLE){ // if below is not null and is available
    el_merge_block_with_above(below); // merge with the block below
  }
  return 0;  
