...
void print_fontified(char *msg, font_t *font){
  ...
  free(breaksIndex); // free the array of breaks index
}
...
font_t *font_load(char *filename){
  ...
  if (file_handle == NULL){ // Check if file is not opened
  font_free(font);
    ...
    while (1){  // run once for every glyph
      ...
      if(ret == EOF){ // if the return value is EOF, break the loop
        ...
        for (int i = 0; i < font->height; i++) { // run once for each line of data in the file
