...
/// Updates by Chris Kauffman <profk@umd.edu>
...
void padstring(char *str, int minlen, char pad){
  int len = strlen(str);
  if(len >= minlen){
    return;
  }
...
void setup(char *infile_name){
  ...
  if(len > 8){
    printf("'%s' is a userID and must be max 8 characters\n",
           userID);
    exit(1);
  }

  padstring(userID, 8, '_');       
